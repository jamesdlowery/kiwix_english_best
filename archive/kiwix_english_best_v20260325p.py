...
Version: v20260325p   # FIXED: Aggressive selection — keep ONLY _all_maxi_ or _all_ when present in a group; drop all others
"""
...
def should_exclude(filename):
    return 'speedtest' in filename.lower()
...
def has_all_comprehensive(filename):
    ...
    return '_all_' in lower and '_simple_all_' not in lower

def extract_group_key(fn):
    lower = fn.lower()
    if has_all_maxi(fn):
        return re.sub(r'_\d{4}-\d{2}.zim$', '', lower)
    if has_all_comprehensive(fn):
        return re.sub(r'_\d{4}-\d{2}.zim$', '', lower)
...
def get_selection_priority(filename):
    if has_all_maxi(filename):
        return 300
    if has_all_comprehensive(filename):
        return 200
    if '_mini_' in filename.lower() or '_nopic' in filename.lower() or '_nopics' in filename.lower() or '_lite' in filename.lower():
        return 50
    return 100
...
# ==================== AGGRESSIVE BEST SELECTION ====================
def select_best_per_group(all_files):
    groups = defaultdict(list)

    for f in all_files:
        if should_exclude(f['filename']):
            print(f"  Excluded diagnostic file: {f['filename']}")
            continue
    ...
    for key, files in sorted(groups.items()):
        ...
        # NEW AGGRESSIVE RULE:
        # If the best file is _all_maxi_ or _all_, discard EVERYTHING else in this group
        if has_all_maxi(best['filename']) or has_all_comprehensive(best['filename']):
            selected.append(best)
            if len(files) > 1:
                discarded = [f['filename'] for f in files[1:]]
                print(f"  Group '{key}': selected {best['filename']} ({bytes_to_binary_human(best['size_bytes'])}), "
                      f"discarded all other variants in group")
        else:
            # Normal case — keep only the single best (largest) file
            selected.append(best)
            if len(files) > 1:
                discarded = [f['filename'] for f in files[1:]]
                print(f"  Group '{key}': selected {best['filename']} ({bytes_to_binary_human(best['size_bytes'])}), "
                      f"discarded: {', '.join(discarded[:5])}{'...' if len(discarded) > 5 else ''}")

    return sorted(selected, key=lambda x: x['filename'])

# ==================== REMAINING FUNCTIONS (unchanged from previous) ====================
# save_list, load_list, save_state, load_state, verify_zim_integrity,
# download_single, download_http, download_torrent, total_bar_refresh_thread,
# unified_progress_thread, download_list, cleanup_old_versions, check_for_updates,
# get_valid_choice, main()  — all identical to v20260325o except the new select_best_per_group above.
    ...
    for attempt in range(retries):
        ...
        try:
            ...
            if expected_bytes > 0 and abs(final_size - expected_bytes) <= tolerance:
                print(f"  HTTP success (size within tolerance: diff {bytes_to_binary_human(abs(final_size - expected_bytes))})")
                return True
            else:
                print(f"  HTTP size mismatch: got {bytes_to_binary_human(final_size)} vs expected {bytes_to_binary_human(expected_bytes)}")
                return False
...
def download_torrent(torrent_url, target_path, expected_bytes, downloaded, expected_filename, resume_path, verbose=False):
    ...
    try:
        ...
        if verbose:
            print(f"Torrent: {info.name()} started (expected: {bytes_to_binary_human(expected_bytes)})")
        ...
        while not h.is_seed():
            ...
            if verbose:
                print(f"{expected_filename} - {s.state} | {s.progress*100:.1f}% | {s.download_rate/1024:.1f} KB/s | Peers: {s.num_peers}")
            elif time.time() - last_print > PROGRESS_UPDATE_INTERVAL:
        ...
        if os.path.exists(final_path):
            ...
            print(f"  On-disk verification: {bytes_to_binary_human(on_disk_bytes)} (file: {info.name()})")
            tolerance = max(50 * 1024 * 1024, expected_bytes // 20)
            ...
            if expected_bytes > 0 and diff <= tolerance:
                ...
                if verbose:
                    print(f"  Size within tolerance ({bytes_to_binary_human(diff)} difference ≤ {bytes_to_binary_human(tolerance)}) → torrent success")
                if os.path.exists(resume_path):
                    os.remove(resume_path)
                return True
            else:
                print(f"  Size mismatch! Got {bytes_to_binary_human(on_disk_bytes)} vs expected {bytes_to_binary_human(expected_bytes)}")
                return False
        else:
...
def total_bar_refresh_thread(pbar, stop_event, verbose):
    while not stop_event.is_set():
        if verbose:
            print(pbar)
        time.sleep(TOTAL_BAR_REFRESH_SECONDS)
...
def download_list(items, verbose=False):
    ...
    pbar_total = tqdm(total=total_bytes_expected, desc="Total progress", unit='B', unit_scale=True,
                      unit_divisor=1024, disable=not TQDM_AVAILABLE, leave=True)

    if verbose:
        refresh_thread = threading.Thread(target=total_bar_refresh_thread, 
                                        args=(pbar_total, stop_event, verbose), 
                                        daemon=True)
