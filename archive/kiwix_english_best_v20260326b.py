...
- Wikipedia: keep ONLY wikipedia_en_all_maxi_* (drop simple_all_maxi, mini, nopic, topic, top, 100, etc.)
- Wiktionary: keep the ONLY available comprehensive version (wiktionary_en_all_nopic_*)
- All other projects: keep _all_maxi_ / _all_ when present; drop ALL _nopic_ versions
- Drop wikivoyage_en_europe_*, speedtest_*, and any unwanted _simple_all_*
"""
...
# ==================== SIZE HANDLING (STRICT BINARY + TWO DECIMAL PLACES) ====================
...
def bytes_to_binary_human(b):
    """Always use the largest sensible binary unit with exactly two decimal places"""
    if b == 0:
        return "0.00 B"
    ...
    for i, unit in enumerate(units):
        if b < 1024 or i == len(units) - 1:
            ...
            return f"{b:.2f} {unit}"
        b /= 1024
    return f"{b:.2f} PiB"  # fallback

# ==================== TIGHT EXCLUSION FILTER (Wiktionary exemption) ====================
def should_exclude(filename):
    lower = filename.lower()

    if 'speedtest' in lower:
        return True
    if lower.startswith('gutenberg_en_lcc-'):
        return True
    if lower.startswith('wikipedia_en_'):
        if '_all_maxi_' not in lower or '_simple_all_' in lower:
            return True
        return False
    # Wiktionary: keep the only comprehensive version
    if lower.startswith('wiktionary_en_all_nopic_'):
        return False
    # Drop ALL other _nopic_ versions
    if '_nopic_' in lower:
        return True
    if 'wikivoyage_en_europe' in lower:
        return True
    if '_simple_all_' in lower and 'wikipedia' not in lower:
        return True
...
def filter_existing_files(best_list):
    ...
    for item in best_list:
        ...
        if key not in existing:
            ...
            print(f"  Skipping already complete file: {item['filename']} ({bytes_to_binary_human(item['size_bytes'])})")
...
def download_single(item, file_index, total_files, verbose=False):
    ...
    if torrent_url and LIBTORRENT_AVAILABLE:
        ...
        if verbose and not torrent_url:
...
def main():
    ...
    if choice == "1":
        ...
        # === NEW: Filter out files that already exist in zims/ ===
        print("\nChecking for already downloaded files...")
        best = filter_existing_files(best)

        if not best:
            print("All best files are already present in the zims/ directory.")
            stats = get_summary_stats()
...
# - After generating best list (before "Save list to file?" prompt), automatically filter out files that already exist in zims/
# - Report and process ONLY the files that still need downloading
# - Cumulative totals and space checks now reflect only remaining files
# - bytes_to_binary_human uses exactly two decimal places
# - Wiktionary exemption for wiktionary_en_all_nopic_*
# 
# This is the production-ready version with all requested output formatting, exclusion rules, and duplicate filtering.
