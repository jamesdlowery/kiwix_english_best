...
# is detected. The refresh thread resets both total_bytes_this_session and
# download_start_time atomically when it sees this flag, ensuring elapsed time
# and speed calculations start fresh from the moment real downloading begins.
reset_session_clock = False
session_elapsed_offset = 0.0  # cumulative elapsed before the last clock reset
...
def total_bar_refresh_thread(pbar, total_expected, stop_event, verbose):
    ...
    while not stop_event.is_set():
        ...
        # FIX 25: If the download_torrent thread signals a checking->downloading
        # transition, reset both the session byte counter and the clock so that
        # elapsed time and speed start from zero at the moment downloading begins.
        if reset_session_clock:
            total_bytes_this_session = 0
            download_start_time = now
            reset_session_clock = False

        # FIX 22: While no network bytes have arrived yet, keep the clock at
        # now so verification time never accumulates in the elapsed denominator.
        if total_bytes_this_session == 0:
            download_start_time = now

        elapsed = now - download_start_time if download_start_time > 0 else 0.0

        # Format elapsed as HH:MM:SS for display in postfix
        ...
        if elapsed > 3.0:
            ...
            pbar.set_postfix_str(f"{elapsed_str}, {speed_str}, ETA={eta_str}")
        else:
            pbar.set_postfix_str(f"{elapsed_str}, ETA=?:??:??")
...
def download_list(items, verbose=False):
    global active_downloads, total_bytes_on_disk, total_bytes_this_session, download_start_time, reset_session_clock
...
#
# Changes in this version (reset elapsed time and session clock on verification->download transition):
#
