...
Version: v20260320d   # Added dual progress bars: per-file + total files completed
    ...
    try:
        ...
        with urllib.request.urlopen(req) as response:
            ...
            with open(local_path, mode) as f:
                # Inner per-file bar (position 1 = below outer bar)
                with tqdm(
                    desc=filename,
                    total=total_size,
                    initial=downloaded,
...
def download_list(items):
    ...
    # Outer total-files progress bar (position 0 = top)
    with tqdm(
        total=total,
        desc="Files",
        unit="file",
