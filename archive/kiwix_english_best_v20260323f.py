...
Version: v20260323f   # ADDED: pre-download free space check + user confirmation on low space
"""
...
SPACE_SAFETY_BUFFER_GB = 10  # minimum extra space required
...
def save_list(items, filename=LIST_FILE):
    ...
    def check_torrent(item):
        ...
        try:
            ...
            r = requests.get(torrent_url, headers=headers, stream=True, timeout=10, allow_redirects=True)
...
def download_single(item, file_index, total_files):
    ...
    if torrent_url and LIBTORRENT_AVAILABLE:
        ...
        success = download_torrent(torrent_url, partial_path, expected_bytes, downloaded, filename, resume_path)
        if success:
    ...
    if not success:
        ...
        success = download_http(http_url, partial_path, expected_bytes, downloaded)
...
def download_http(url, target_path, expected_bytes, downloaded):
    ...
    for attempt in range(retries):
        try:
            ...
            print(f"  HTTP attempt {attempt+1}/{retries} (range: {range_header or 'full'}) from {url}")
            ...
            print(f"  HTTP timeout (attempt {attempt+1}/{retries})")
            time.sleep(backoff[attempt] if attempt < len(backoff) else 30)
        except Exception as e:
            print(f"  HTTP download error: {e}")
            time.sleep(backoff[attempt] if attempt < len(backoff) else 30)
    print("  HTTP failed after retries")
    return False
...
def download_list(items):
    ...
    # Pre-check: identify files that actually need downloading
    items_to_download = []
    for item in items:
    ...
    with tqdm(total=total_bytes_expected, desc="Total progress", unit='B', unit_scale=True, unit_divisor=1024,
        ...
        def wrapped_download(item, idx):
            success = download_single(item, idx, total_files)
            if success:
                nonlocal completed
                completed += 1
...
def main():
    ...
    if choice == "1":
        ...
        # Pre-download space check for fresh list
        ...
        # Pre-download space check for resume
        items_to_download = []
        for item in items:
            target_path = os.path.join(ZIM_SUBFOLDER, item['filename'])
            if not os.path.exists(target_path) or os.path.getsize(target_path) != item['size_bytes']:
        ...
        # Pre-download space check for updates
