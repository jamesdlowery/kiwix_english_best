    ...
    while not stop_event.is_set():
        ...
        if now - last_print >= TOTAL_BAR_REFRESH_SECONDS:
            ...
            if verbose:
                ...
                # Move cursor up over the previous block and overwrite each
                # line cleanly before printing the new block.
                if last_block_lines > 0:
                    # Move up past the blank line, then erase+overwrite each line
...
#
# Changes in this version (in-place rewrite for non-verbose mode):
#
# FIX 14 -- Non-verbose mode now renders the same block format as verbose
#   (file lines + total bar + blank line) but rewrites the block in place
#   each refresh cycle using ANSI cursor-up + erase-line sequences, so the
#   terminal stays clean and doesn't scroll. unified_progress_thread has been
#   removed entirely -- total_bar_refresh_thread now handles both modes:
#   - verbose=True:  prints each block as scrolling output (FIX 13 behaviour)
#   - verbose=False: moves cursor up over the previous block and overwrites
#                    each line with ERASE_LINE before printing the new block
#
