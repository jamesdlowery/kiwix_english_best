...
def download_list(items, verbose=False, cleanup_after_each=False):
    ...
    try:
        ...
        # FIX 44: Report any .partial files left in zims/ so the user knows
        # which downloads need to be resumed and how to do it.
        partials = [f for f in os.listdir(ZIM_SUBFOLDER) if f.endswith('.partial')]
        if partials:
            print(f"\n  ⚠ Incomplete download(s) — {len(partials)} partial file(s) in zims/:")
            for p in sorted(partials):
                ppath = os.path.join(ZIM_SUBFOLDER, p)
                psize = os.path.getsize(ppath)
                print(f"    {p} ({bytes_to_binary_human(psize)} on disk so far)")
            print(f"    → Run option 2 to resume these downloads.")
...
#
# Changes in this version (fix post-run summary overwritten by in-place rewrite):
#
