...
def total_bar_refresh_thread(pbar, total_expected, stop_event, verbose):
    ...
    while not stop_event.is_set():
        now = time.time()
        elapsed = now - download_start_time if download_start_time > 0 else 0
        if elapsed > 1.0:
            ...
            eta_str = f"{int(eta_sec//3600):02d}:{int((eta_sec%3600)//60):02d}:{int(eta_sec%60):02d}" if eta_sec < 86400 else "??:??:??"
        else:
            eta_str = "?:??:??"
        pbar.n = total_bytes_downloaded
        pbar.refresh()
...
# - Total progress bar now updates LIVE every 0.5s from actual bytes downloaded (torrent + HTTP)
# - Byte-based ETA calculation with clean HH:MM:SS format
# - Faster refresh interval (2s) in verbose mode for smoother feel
# - download_start_time added for accurate speed/ETA from first byte
# 
# The Total progress line should now advance continuously and show a reliable ETA.
