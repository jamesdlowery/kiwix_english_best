    ...
    try:
        ...
        while not h.is_seed():
            ...
            if downloaded_this_tick > 0:
                ...
                # FIX 7: Only credit bytes to the global counter when libtorrent
                # is actually pulling from the network. During checking phases,
                # total_wanted_done advances as existing pieces are verified on
                # disk -- counting those as downloaded inflates the speed
                # calculation and makes the ETA wildly optimistic at the start
                # of a resumed session.
                if state_str not in CHECKING_STATES:
                    total_bytes_downloaded += downloaded_this_tick
            with progress_lock:
...
def total_bar_refresh_thread(pbar, total_expected, stop_event, verbose):
    ...
    while not stop_event.is_set():
        ...
        # Force update with current downloaded bytes
        pbar.n = min(total_bytes_downloaded, total_expected)
        pbar.refresh()

        if elapsed > 3.0:  # Start ETA earlier
            speed = total_bytes_downloaded / elapsed
            remaining = total_expected - total_bytes_downloaded
...
def download_list(items, verbose=False):
    global active_downloads, total_bytes_downloaded, download_start_time
    active_downloads.clear()
    download_start_time = time.time()

    # FIX 8: Seed the progress counter from any partial files already on disk
    # so the total bar and ETA start from an honest baseline rather than 0.
    # Without this, a resumed session looks like it starts from scratch and the
    # early speed/ETA figures are skewed by however much was already downloaded.
    already_on_disk = 0
    for item in items:
        partial = os.path.join(ZIM_SUBFOLDER, f"{item['filename']}.partial")
        if os.path.exists(partial):
            already_on_disk += os.path.getsize(partial)
    total_bytes_downloaded = already_on_disk
    ...
    try:
        def wrapped_download(item, idx):
            ...
            if success:
                global total_bytes_downloaded
                with progress_lock:
                    total_bytes_downloaded = max(total_bytes_downloaded, total_bytes_expected)
...
#
# Changes in this version (speed/ETA accuracy for resumed sessions):
#
# FIX 7 -- download_torrent: total_bytes_downloaded is now only incremented
#   when libtorrent is in a genuine network-downloading state. Previously, bytes
#   were counted during checking_files/checking_resume_data as libtorrent
#   advanced total_wanted_done through disk verification -- this made the speed
#   read as hundreds of MB/s (disk speed) and the ETA wildly optimistic at the
#   start of a resumed session.
#
# FIX 8 -- download_list: total_bytes_downloaded is now seeded at startup with
#   the combined size of any .partial files already on disk. Without this,
#   resumed sessions always started the progress bar and ETA from 0, as if
#   nothing had been downloaded yet, producing a misleading baseline.
#
# Earlier fixes carried forward from v20260328a and v20260328b:
