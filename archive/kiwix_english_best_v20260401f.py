...
def check_for_updates():
    ...
    for key, disk_item in on_disk.items():
        ...
        # FIX 40: Compare content dates parsed from filenames on both sides.
        # curr['date'] is the Apache directory modification timestamp, which
        # is unrelated to the content date in the filename and will almost
        # always be newer than the on-disk filename date, causing everything
        # to be flagged as an update. Use parse_date_from_filename() on both
        # sides so we're comparing YYYY-MM content dates consistently.
        curr_content_date = parse_date_from_filename(curr['filename'])
        disk_content_date = disk_item['date']  # already from parse_date_from_filename
        is_newer = (
            curr_content_date > disk_content_date or
            curr['size_bytes'] > disk_item['size_bytes'] * UPDATE_SIZE_THRESHOLD
        )
        if is_newer:
            updates.append(curr)
...
#
# Changes in this version (fix option 3 date comparison):
#
