...
def verify_zim_integrity(target_path, filename, via_torrent=False):
    ...
    # FIX 31: Torrent downloads are already piece-verified by libtorrent against
    # the torrent manifest during download. Running zimcheck on top is redundant
    # and prohibitively slow for large files. For torrent downloads, just confirm
    # the ZIM magic number header and trust libtorrent's verification.
    if via_torrent:
        try:
            with open(target_path, 'rb') as f:
                if f.read(4) == b'\x04\x00\x00\x00':
                    ...
                    return True
            print(f"  ZIM header check FAILED for {filename} (bad magic number)")
            corrupt_dir = os.path.join(ZIM_SUBFOLDER, "corrupt")
            os.makedirs(corrupt_dir, exist_ok=True)
            shutil.move(target_path, os.path.join(corrupt_dir, base_name))
            return False
        except Exception as e:
            print(f"  ZIM header check error for {filename}: {e}")
            return False

    # HTTP downloads: run zimcheck with a timeout scaled to file size.
    # FIX 31: Allow ~2 seconds per GiB, minimum 300 s, maximum 3600 s.
    try:
        file_size_gb = os.path.getsize(target_path) / (1024 ** 3)
        ...
        # FIX 31: Timeout is not proof of corruption -- the file is left in
        # place with a warning rather than being discarded or moved to corrupt/.
        print(f"  WARNING: zimcheck timed out for {filename} after {timeout_sec}s")
        print(f"  File kept at {target_path} -- run zimcheck manually to verify")
...
#
# Changes in this version (smarter integrity checking):
#
