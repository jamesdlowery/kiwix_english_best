...
def main():
    ...
    if choice == "1":
        ...
        to_download = check_for_updates()
        if not to_download:
            print("No updates or new groups found.")
            return
        updates    = [f for f in to_download if extract_group_key(f['filename']) in
                      {extract_group_key(fn) for fn in os.listdir(ZIM_SUBFOLDER)
                       if fn.endswith('.zim')}]
        new_groups = [f for f in to_download if f not in updates]
...
#
# Changes in this version (per-file cleanup + updated menu text):
#
