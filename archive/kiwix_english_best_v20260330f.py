...
#
# Changes in this version (fix erroneous file count and missing blank line on resume):
#
# FIX 21 -- menu option 2: items_to_download filter now uses the same tolerance
#   check as download_single (abs(on_disk - expected) <= max(50MB, expected/20))
#   instead of an exact size equality check. Torrent-completed files whose
#   on-disk size differs slightly from item['size_bytes'] were incorrectly
#   included in the download list, inflating the "Files to download" count and
#   the required-space figure. Also added a blank line before the verbose prompt
#   to separate it visually from the space check block.
#
