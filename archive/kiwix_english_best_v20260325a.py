...
Version: v20260325a   # ADDED: unified progress line in quiet mode (every 5s)
"""
...
def download_single(item, file_index, total_files, verbose=False):
    ...
    if torrent_url and LIBTORRENT_AVAILABLE:
        ...
        if not LIBTORRENT_AVAILABLE and verbose:
...
def download_torrent(torrent_url, target_path, expected_bytes, downloaded, expected_filename, resume_path, verbose=False):
    ...
    try:
        ...
        while not h.is_seed():
            ...
            # Update shared progress for unified line (quiet mode)
            with progress_lock:
            ...
            with progress_lock:
                if active_downloads:
                    ...
                    for name, data in list(active_downloads.items()):
                        parts.append(f"{name[:20]} {data['progress']:.1f}% · {data['rate']:.1f} KB/s")
...
def download_list(items, verbose=False):
    ...
    # Start unified progress thread for quiet mode
