...
TORRENT_CHECK_TIMEOUT = 10  # increased
...
def download_single(item, file_index, total_files):
    ...
    if torrent_url and LIBTORRENT_AVAILABLE:
        ...
        success = download_torrent(torrent_url, target_path, expected_bytes, downloaded, filename)
        if success:
...
def download_torrent(torrent_url, target_path, expected_bytes, downloaded, expected_filename):
    if not LIBTORRENT_AVAILABLE:
    ...
    try:
        ...
        if os.path.exists(final_path):
            ...
            tolerance = max(1024*1024 * 10, expected_bytes // 100)  # 10 MiB min
            if expected_bytes > 0 and abs(on_disk_bytes - expected_bytes) <= tolerance:
                if final_path != target_path:
                    os.rename(final_path, target_path)
                    print(f"  Renamed {info.name()} → {expected_filename}")
                print(f"  Size matches expected (within tolerance) → torrent success")
                return True
            else:
                print(f"  Size mismatch! Got {on_disk_bytes} B, expected {expected_bytes} B (±{tolerance} B allowed)")
                return False
        else:
