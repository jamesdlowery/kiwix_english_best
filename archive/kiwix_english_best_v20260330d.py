...
#
# Changes in this version (fix 100% baseline overshoot on resume):
#
# FIX 19 -- download_list: completed .zim files now contribute item['size_bytes']
#   to the already_on_disk baseline rather than os.path.getsize(). The actual
#   on-disk file size can exceed the catalogued size due to torrent piece-boundary
#   rounding or tolerance margins, which caused already_on_disk to overshoot
#   total_bytes_expected and clamp the bar to 100% before downloading anything.
#   In-progress .partial files still use os.path.getsize() (their real size is
#   the correct resume offset and will always be less than item['size_bytes']).
#
