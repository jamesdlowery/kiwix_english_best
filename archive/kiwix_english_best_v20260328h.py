...
def total_bar_refresh_thread(pbar, total_expected, stop_event, verbose):
    ...
    last_print = 0
    # Column widths for tidy verbose output
    NAME_COL = 60
    STATE_COL = 28
    while not stop_event.is_set():
        ...
        # FIX 13: In verbose mode, print all file lines + total bar as one
        # atomic block per refresh cycle. Nothing else prints in verbose mode,
        # so frames are always tidy and never interleaved.
        if verbose and now - last_print >= TOTAL_BAR_REFRESH_SECONDS:
            ...
            for name, data in snapshot:
                ...
                # Name + state in fixed columns, then stats
                label = f"{name} - {state}"
                if checking:
                    line = f"{label:<{NAME_COL + STATE_COL}} | {pct:5.1f}%"
                else:
                    line = (f"{label:<{NAME_COL + STATE_COL}} | {pct:5.1f}%"
                            f" | {rate:>8.1f} KB/s | {method}")
                lines.append(line)
            # Print file lines, then the total bar, then a blank line
            if lines:
                print('\n'.join(lines))
            print(pbar)
            print()
...
#
# Changes in this version (tidy atomic verbose output blocks):
#
# FIX 13 -- Verbose output is now owned entirely by total_bar_refresh_thread,
#   which prints all currently-downloading file lines followed immediately by
#   the total progress bar and a blank line as one atomic block per refresh
#   cycle. Previously, each worker thread and the refresh thread printed
#   independently, causing chaotic interleaving. Changes:
#   - download_torrent: removed independent verbose prints; active_downloads
#     entries now include 'state', 'method' (e.g. "Peers: 33"), and 'checking'
#     fields so the refresh thread can render each file line correctly.
#   - download_http: removed independent verbose prints; now updates
#     active_downloads with progress, rate, state='Downloading', method='HTTP'.
#   - total_bar_refresh_thread: assembles all file lines from active_downloads
#     snapshot + total bar into a single print() call per cycle. Output format:
#       <filename> - <state>     | <pct>% | <rate> KB/s | <method>
#       <filename> - <state>     | <pct>% | <rate> KB/s | <method>
#       Total progress: ...
#
