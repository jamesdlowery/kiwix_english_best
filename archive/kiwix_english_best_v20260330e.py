...
#
# Changes in this version (fix erroneous insufficient-space warning on resume):
#
# FIX 20 -- get_required_space_gb: now deducts bytes already on disk for each
#   item before summing required space. Previously the full item['size_bytes']
#   was always used regardless of what was already downloaded, so resuming a
#   half-done session would report insufficient space even when plenty was
#   available. Logic:
#   - Completed .zim present:  needs 0 additional space
#   - .partial present:        needs item['size_bytes'] - partial size
#   - Nothing on disk:         needs full item['size_bytes']
#
