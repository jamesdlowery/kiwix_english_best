...
Version: v20260325o   # FIXED: Precise grouping so only _all_maxi_ / _all_ are kept for Gutenberg & Wikipedia; splits kept separate
"""
...
def get_selection_priority(filename):
    ...
    if '_all_maxi_' in fn:
        return 300
    if '_all_' in fn and '_simple_all_' not in fn:   # treat simple_all as normal
        return 200
    if '_mini_' in fn or '_nopic' in fn or '_nopics' in fn or '_lite' in fn:
        return 50
    return 100

def extract_group_key(fn):
    """Much more precise grouping to separate _all_ from splits"""
    lower = fn.lower()
    # Special cases for comprehensive files
    if '_all_maxi_' in lower:
        return re.sub(r'_\d{4}-\d{2}.zim$', '', lower)
    if '_all_' in lower and '_simple_all_' not in lower:
        return re.sub(r'_\d{4}-\d{2}.zim$', '', lower)
    if '_simple_all_' in lower:
        return re.sub(r'_\d{4}-\d{2}.zim$', '', lower)

    # For Gutenberg lcc- and Wikipedia topic-specific / top / wp1 / 100
    # Keep the full variant name so they stay in separate groups
    ...
    for key, files in sorted(groups.items()):
        ...
        if len(files) > 1:
            ...
            print(f"  Group '{key}': selected {best['filename']} ({bytes_to_binary_human(best['size_bytes'])}), "
                  f"discarded: {', '.join(discarded[:6])}{'...' if len(discarded) > 6 else ''}")

    return sorted(selected, key=lambda x: x['filename'])

# ==================== REMAINING FUNCTIONS (save_list, load_list, etc.) ====================
# (All remaining functions are identical to v20260325n except for the updated extract_group_key and select_best_per_group above)
