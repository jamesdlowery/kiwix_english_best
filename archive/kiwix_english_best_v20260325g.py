...
Version: v20260325g   # FIXED: verbose mode now uses separate lines per file (no bottom-line flicker)
"""
...
# Shared state for clean shutdown and unified progress
...
def download_torrent(torrent_url, target_path, expected_bytes, downloaded, expected_filename, resume_path, verbose=False):
    ...
    try:
        ...
        while not h.is_seed():
            ...
            if verbose:
                # In verbose mode we print on its own line (no \r)
                print(f"\n{expected_filename} - {s.state} | {s.progress*100:.1f}% | {s.download_rate/1024:.1f} KB/s | Peers: {s.num_peers}")
