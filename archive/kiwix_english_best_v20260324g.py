...
Version: v20260324g   # ADDED: unified progress line in quiet mode (every 5s) while keeping verbose unchanged
"""
...
PROGRESS_UPDATE_INTERVAL = 5  # seconds for unified progress line in quiet mode
...
# Global for unified progress in quiet mode
active_status = {}
...
def verify_zim_integrity(target_path, filename):
    ...
    try:
        ...
        print(f"  ZIM integrity check timed out for {filename}")
        return False
...
def download_single(item, file_index, total_files, verbose=False):
    ...
    if os.path.exists(target_path):
        ...
        if on_disk == expected_bytes:
            ...
            print(f"  Pre-existing file size mismatch ({bytes_to_human(on_disk)} vs {bytes_to_human(expected_bytes)}) — will re-download")
...
def download_torrent(torrent_url, target_path, expected_bytes, downloaded, expected_filename, resume_path, verbose=False):
    ...
    try:
        ...
        while not h.is_seed():
            ...
            # Update global status for unified progress in quiet mode
            with status_lock:
                active_status[expected_filename] = {
...
def unified_progress_thread(total_files, stop_event):
    last_update = 0
    while not stop_event.is_set():
        if time.time() - last_update >= PROGRESS_UPDATE_INTERVAL:
            with status_lock:
                if active_status:
                    parts = []
                    for fn, stats in list(active_status.items()):
                        parts.append(f"{fn} {stats['progress']:.1f}% · {stats['rate']:.1f} KB/s")
                    line = " | ".join(parts)
                    print(f"\rUnified progress ({len(active_status)} active): {line}", end='')
                    sys.stdout.flush()
...
def download_list(items, verbose=False):
    ...
    if not items_to_download:
        print("All files already complete — nothing to download.")
    ...
    if not verbose:
        progress_thread = threading.Thread(target=unified_progress_thread, args=(total_files, stop_event), daemon=True)
    ...
    with tqdm(total=total_bytes_expected, desc="Total progress", unit='B', unit_scale=True, unit_divisor=1024,
        ...
        def wrapped_download(item, idx):
            ...
            if success:
                ...
                completed += 1
                with status_lock:
                    if item['filename'] in active_status:
                        del active_status[item['filename']]
    ...
    stop_event.set()
    if not verbose and 'progress_thread' in locals():
