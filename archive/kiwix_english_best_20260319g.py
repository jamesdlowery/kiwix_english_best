...
Version: v20260319g   # New groups = updates + automatic cleanup of older versions
Purpose: Crawl Kiwix, select best English ZIMs, download/resume, check for updates
         (now includes new groups) and auto-delete older files in the same group.
...
UPDATE_SIZE_THRESHOLD = 1.05  # only count size increase if >5%
...
def parse_date_from_filename(filename):
    """Extract YYYY-MM from filename for comparison."""
...
def check_for_updates():
    ...
    updates = []      # changed groups
    new_groups = []   # completely new groups

    # Check existing groups for updates
    for old in old_items:
        key = old['group_key']
        if key not in current_dict:
            continue
        curr = current_dict[key]
        old_date = old.get('date', datetime.min) if 'date' in old else parse_date_from_filename(old['filename'])
        curr_date = curr['date']
...
def download_list(items):
    ...
    for item in items:
        success = download_file(item['url'], item['filename'], item['size_bytes'])
        if not success:
            print(f"Skipping cleanup for failed file: {item['filename']}")
...
def main():
    ...
    if choice == "1":
        ...
        if input("Download updates/new files now? (y/n): ").lower() == 'y':
            ...
            if input("Automatically clean up older versions? (y/n): ").lower() == 'y':
