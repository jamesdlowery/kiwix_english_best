...
def download_list(items, verbose=False):
    ...
    # FIX 10: Use a custom bar_format that omits tqdm's built-in {rate} field.
    # tqdm calculates rate from pbar.n / elapsed, which includes the partial-file
    # baseline bytes we seed at startup, producing an inflated figure like
    # 188 MB/s even when the actual network rate is much lower. By removing
    # {rate} from the format string and injecting our own session-only rate via
    # set_postfix(), the displayed rate is always honest.
    BAR_FMT = (
...
#
# Changes in this version (honest rate display in Total progress bar):
#
# FIX 10 -- tqdm's built-in {rate} field is now suppressed via a custom
#   bar_format string that omits it entirely. tqdm computes its rate from
#   pbar.n / elapsed; since pbar.n is seeded with the partial-file baseline
#   at startup, tqdm's rate was inflated (e.g. 188 MB/s) even when actual
#   network throughput was ~1 MB/s. The refresh thread now injects its own
#   session-only rate (total_bytes_this_session / elapsed) directly into the
#   postfix string alongside ETA, so both figures are always honest.
#   Example output: "1.24 MiB/s, ETA=01:23:45"
#
