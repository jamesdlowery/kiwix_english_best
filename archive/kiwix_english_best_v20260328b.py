...
#
# Changes in this version (torrent progress display improvements):
#
# FIX 5 — download_torrent: libtorrent state strings are now mapped to
#   human-readable labels (e.g. 'checking_files' -> 'Verifying existing data',
#   'downloading' -> 'Downloading'). Previously raw internal enum strings were
#   printed directly, which were cryptic to users.
#
# FIX 6 — download_torrent: during verification phases (checking_files,
#   checking_resume_data, downloading_metadata, allocating), the KB/s rate and
#   Peers count are now suppressed from verbose output. Those fields are always
#   0 during these phases because libtorrent hasn't connected to peers yet --
#   displaying them implied something was broken when it wasn't.
#
# Note: v20260328a (CC fix of G v20260327d) contained fixes 1-4:
