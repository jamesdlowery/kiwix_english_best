...
def verify_zim_integrity(target_path, filename, via_torrent=False):
    ...
    # FIX 31: Torrent downloads are already piece-verified by libtorrent.
    # Just confirm the ZIM magic number header.
    if via_torrent:
        try:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(detail[:500])   # show first 500 chars on screen
            log_failure(filename,
                        f"zimcheck -C failed (exit {result.returncode})",
        ...
        # FIX 31: Timeout is not proof of corruption -- leave file in place.
        reason = f"zimcheck timed out after {timeout_sec}s"
        print(f"  WARNING: {reason} for {filename}")
        print(f"  File kept at {target_path} -- run 'zimcheck -C -D {target_path}' manually")
        log_failure(filename, reason,
                    f"Timeout after {timeout_sec}s -- file NOT moved, verify manually",
...
#
# Changes in this version (failure logging + magic number fix + zimcheck -D):
#
