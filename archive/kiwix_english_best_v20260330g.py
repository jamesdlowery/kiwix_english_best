...
def get_required_space_gb(items_to_download):
    # FIX 20: Deduct bytes already on disk for each item so the space check
    # reflects only what still needs to be downloaded, not the full file sizes.
    # A completed .zim counts as fully satisfied; a .partial counts its current
    # size as already written. Without this, resuming a half-done session would
    # always report insufficient space even when plenty is available.
    remaining_bytes = 0
    for item in items_to_download:
        ...
        if os.path.exists(zim_path):
            ...
            remaining_bytes += max(0, item['size_bytes'] - os.path.getsize(partial_path))
        else:
...
def download_list(items, verbose=False):
    ...
    # FIX 19: Seed total_bytes_on_disk carefully to avoid overshooting
    # total_bytes_expected (which is built from item['size_bytes']).
    # - Completed .zim files: credit item['size_bytes'], NOT os.path.getsize().
    #   Actual on-disk size can differ slightly from the catalogued size (torrent
    #   piece boundaries, tolerance rounding), so using the real size could make
    #   already_on_disk > total_bytes_expected, clamping the bar to 100% before
    #   any downloading begins.
    # - In-progress .partial files: use os.path.getsize() -- these are genuinely
    #   incomplete and their real size is the correct resume offset.
    already_on_disk = 0
    for item in items:
        zim_path     = os.path.join(ZIM_SUBFOLDER, item['filename'])
        partial_path = os.path.join(ZIM_SUBFOLDER, f"{item['filename']}.partial")
        if os.path.exists(zim_path):
            ...
            already_on_disk += os.path.getsize(partial_path)  # real partial size
    total_bytes_on_disk = already_on_disk
    total_bytes_this_session = 0
    if already_on_disk > 0:
        print(f"Resuming: {bytes_to_binary_human(already_on_disk)} already on disk across completed/partial files.")
...
#
# Changes in this version (fix speed/ETA inflated by verification phase elapsed time):
#
# FIX 22 -- total_bar_refresh_thread: download_start_time is now reset to the
#   current time on every refresh cycle while total_bytes_this_session is still
#   zero. Previously the clock started when download_list() was called, so all
#   time spent in the torrent verification phase (checking_files etc.) accumulated
#   in the elapsed denominator. When actual downloading began, the first few bytes
#   were divided by several minutes of elapsed time, producing an inflated speed
#   (e.g. 3.19 MiB/s) and a falsely optimistic ETA. Now the clock effectively
#   starts at the moment the first real network byte arrives.
#
