...
Version: v20260324a   # March 24, 2026 - first increment of the day
"""
...
def fetch_directory(url, depth=0, visited=None, collected=None):
    ...
    try:
        ...
        for line in lines:
            ...
            if True:  # Removed English-only filter to support all languages
...
def select_best_per_group(all_files):
    ...
    VARIANT_SUFFIXES = [
        r'_maxi', r'_nopic', r'_nopics', r'_minimal', r'_min', r'_lite',
...
def select_languages(lang_tree):
    ...
    try:
        ...
        return selected_langs if selected_langs else languages
    except:
...
# ==================== DOWNLOAD LIST WITH LANGUAGE + TREE ====================
def download_list(items, verbose=False):
    ...
    lang_tree = build_language_tree(items)
    selected_langs = select_languages(lang_tree)

    # 2. Build filtered items for selected languages
    filtered_items = []
    for lang in selected_langs:
        for top in lang_tree[lang]:
            for sub in lang_tree[lang][top]:
                filtered_items.extend(lang_tree[lang][top][sub])

    print(f"\nSelected {len(filtered_items)} files from {len(selected_langs)} language(s).")

    # 3. Subcategory tree selection
    tree = build_category_tree_for_languages(filtered_items, selected_langs)
    final_items = select_from_tree(tree)

    print(f"\nFinal selection: {len(final_items)} files")
    ...
    if not items_to_download:
        print("All selected files already complete — nothing to download.")
        stats = get_summary_stats()
        print("\nPost-run summary (no new downloads):")
        print(f"  - Total files in list: {len(final_items)}")
    ...
    print(f"\nSpace check:")
    print(f"  Files to download: {len(items_to_download)} ({bytes_to_human(sum(i['size_bytes'] for i in items_to_download))})")
    print(f"  Required (incl. {SPACE_SAFETY_BUFFER_GB} GB buffer): ~{required_gb:.1f} GB")
    print(f"  Available in {ZIM_SUBFOLDER}: ~{available_gb:.1f} GB")
    ...
    total_bytes_expected = sum(item['size_bytes'] for item in final_items)

    print(f"\nStarting parallel download of {len(final_items)} files "
          f"({bytes_to_human(total_bytes_expected)} total, max {MAX_CONCURRENT_DOWNLOADS} concurrent)... (Ctrl+C to pause)\n")

    remaining = final_items[:]
    ...
    with tqdm(total=total_bytes_expected, desc="Total progress", unit='B', unit_scale=True, unit_divisor=1024,
        ...
        def wrapped_download(item, idx):
            success = download_single(item, idx, len(final_items), verbose=verbose)
            if success:
                nonlocal completed
                completed += 1
                ...
                print(f"Files completed: {completed} / {len(final_items)}\n")

        with ThreadPoolExecutor(max_workers=MAX_CONCURRENT_DOWNLOADS) as executor:
            futures = [executor.submit(wrapped_download, item, i) for i, item in enumerate(final_items, 1)]
    ...
    print(f"\nAll {len(final_items)} files completed!")

# ==================== CLEANUP & UPDATE CHECKER (unchanged) ====================
...
def main():
    ...
    if choice == "1":
        ...
        print(f"\nSelected {len(best)} best files before language filter.")
