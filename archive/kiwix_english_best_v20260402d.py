...
def download_single(item, file_index, total_files, verbose=False):
    ...
    if not success and not stop_event.is_set():
        success = download_http(http_url, partial_path, expected_bytes, downloaded, verbose, file_index)
    if success:
        ...
        with progress_lock:
            active_downloads.pop(filename, None)
        # FIX 44: Report the partial file and tell the user how to resume.
        if os.path.exists(partial_path):
            ...
            print(f"    Run option 2 to resume this download.")
        else:
            print(f"  ✗ Failed: {filename} ({file_index}/{total_files}) — no partial file saved")
        return False
    return False
...
def main():
    ...
    if choice == "1":
        ...
        if download_choice == "y":
            ...
            # FIX 47: After the session, write a resume file for any items that
            # did not complete. If all completed, delete any existing resume file.
            expected_sizes = {item['filename']: item['size_bytes'] for item in to_download}
            incomplete_items = []
            for item in to_download:
                fname = item['filename']
                zim_path = os.path.join(ZIM_SUBFOLDER, fname)
                partial_path = os.path.join(ZIM_SUBFOLDER, f"{fname}.partial")
                if os.path.exists(partial_path) and not os.path.exists(zim_path):
                    incomplete_items.append(item)
            if incomplete_items:
                save_resume_file(incomplete_items)
            else:
                delete_resume_file()
    elif choice == "4":
        os.makedirs(ZIM_SUBFOLDER, exist_ok=True)
...
#
# Changes in this version (rename resume file):
#
