...
#
# Changes in this version (fix rogue second bar line and first-block offset):
#
# FIX 15 -- Two root causes of the double "Total progress" line and the
#   first-file-repeated-on-three-lines issue:
#
#   (a) pbar.refresh() was being called in the refresh loop, causing tqdm to
#       write the bar to its output stream independently of our block renderer,
#       producing a rogue second bar line. Removed entirely -- we render via
#       str(pbar) inside our block and tqdm never writes to the terminal itself.
#
#   (b) tqdm's output stream is now routed to an io.StringIO() sink so any
#       remaining internal tqdm writes (e.g. from pbar.close() at shutdown)
#       are silently discarded rather than appearing on-screen.
#
#   (c) last_block_lines is now set immediately after every block print,
#       including the very first one, so the cursor-up count on the second
#       cycle is always correct and doesn't leave orphaned lines at the top.
#
