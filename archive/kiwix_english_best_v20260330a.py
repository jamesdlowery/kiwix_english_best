...
def download_list(items, verbose=False):
    ...
    # FIX 16: Seed total_bytes_on_disk from ALL files already on disk --
    # both completed .zim files and in-progress .partial files. Previously
    # only .partial files were counted, so if a session was interrupted after
    # some files completed (e.g. due to a power outage), the completed files'
    # bytes were missing from the baseline. total_bytes_expected still included
    # their full sizes, making the bar report 100% before downloading anything.
    already_on_disk = 0
    for item in items:
        ...
        if os.path.exists(zim_path):
            already_on_disk += os.path.getsize(zim_path)
        elif os.path.exists(partial_path):
            already_on_disk += os.path.getsize(partial_path)
...
#
# Changes in this version (correct baseline seeding after partial-session interruption):
#
# FIX 16 -- download_list: the total_bytes_on_disk baseline now counts both
#   completed .zim files AND in-progress .partial files for every item in the
#   download list. Previously only .partial files were counted, so if a session
#   was interrupted after some files completed (e.g. power outage), the
#   completed files' bytes were absent from the baseline while still being
#   included in total_bytes_expected -- causing the progress bar to report
#   100% and a negative ETA before any downloading had begun.
#   Priority: .zim (completed) takes precedence over .partial (in-progress)
#   for each item, so double-counting is not possible.
#
# Earlier fixes carried forward from v20260328a through v20260328j:
