...
Version: v20260325q   # FINAL: Hard drop rules for all unwanted Gutenberg lcc-* and all Wikipedia except _all_maxi_
"""
...
def should_exclude(filename):
    ...
    if 'speedtest' in lower:
        return True
    # Gutenberg: drop all lcc- splits, keep only _all_
    if lower.startswith('gutenberg_en_lcc-'):
        return True
    # Wikipedia: keep ONLY _all_maxi_; drop everything else that starts with wikipedia_en_
    if lower.startswith('wikipedia_en_') and not '_all_maxi_' in lower:
        return True
    return False
...
# (load_list, save_state, load_state, verify_zim_integrity, download_single, download_http, download_torrent,
# total_bar_refresh_thread, unified_progress_thread, download_list, cleanup_old_versions, check_for_updates,
# get_valid_choice, main() remain exactly the same as in v20260325p)
