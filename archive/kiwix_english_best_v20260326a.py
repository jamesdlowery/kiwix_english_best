...
def main():
    ...
    if choice == "1":
        ...
        print(f"\nSelected {len(best)} best files ({bytes_to_binary_human(total_size)}):")
        for f in best:
            print(f"  {f['filename']} ({bytes_to_binary_human(f['size_bytes'])})")

        print(f"\nTotal selected: {len(best)} files ({bytes_to_binary_human(total_size)})")
...
# - Version bumped to v20260326a to reflect current date
# - bytes_to_binary_human now always uses exactly two decimal places (e.g. 1.21 TiB, 160.00 MiB)
# - All size displays in STDOUT are now consistently formatted with two decimals
# - Cumulative total shown at start and end of "Selected X best files" list
# - Wiktionary exemption for wiktionary_en_all_nopic_*
# - Strict binary scaling (GiB → TiB when appropriate)
# 
# This is the production-ready version with all requested output formatting and exclusion rules.
