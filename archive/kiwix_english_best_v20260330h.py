...
#
# Changes in this version (fix misleading partial-file size reporting):
#
# FIX 23 -- libtorrent may use sparse file allocation, meaning os.path.getsize()
#   on a .partial file can return a tiny value (e.g. 5 MiB) even when 60%+ of
#   the content has been downloaded. Two places were affected:
#
#   (a) download_list baseline seeding: .partial files are no longer seeded into
#       total_bytes_on_disk at startup. Their verified size is populated naturally
#       by the total_bytes_on_disk += downloaded_this_tick increment in
#       download_torrent as libtorrent works through verification. The startup
#       message now says "Calculating data already on disk across existing
#       partial file(s)..." rather than reporting a misleading size.
#
#   (b) get_required_space_gb: .partial files are no longer credited as partially
#       satisfying space requirements via os.path.getsize(). They are treated
#       conservatively as needing their full item['size_bytes']. Only completed
#       .zim files (reliable size) are credited as needing 0 additional space.
#
