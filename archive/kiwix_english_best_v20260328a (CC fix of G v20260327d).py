...
#
# Bug fixes in this version (all related to large-file download failures):
#
# FIX 1 — download_http: mode ('ab'/'wb') now determined inside the retry loop,
#   after any 416-triggered reset of `downloaded`. Previously, a 416 would reset
#   `downloaded` to 0 but mode had already been set to 'ab', causing corrupted
#   appends on the next attempt.
#
# FIX 2 — download_http: timeout changed from a flat 30 s to (30, 300) — i.e.
#   30 s to connect, 300 s between chunks. The original 30 s read timeout would
#   kill slow but healthy large-file transfers mid-stream.
#
# FIX 3 — download_single: .partial file is now renamed to .zim BEFORE
#   verify_zim_integrity is called. Previously, passing the .partial path meant
#   zimcheck could move a good file into corrupt/ under the wrong name, and the
#   download was permanently discarded even though the data was fine.
#
# FIX 4 — download_http: mirror URL substitution now always starts from the
#   original URL (renamed parameter `original_url`). Previously, each retry
#   would mutate the already-substituted URL, compounding replacements and
#   potentially producing malformed URLs by attempt 3+.
