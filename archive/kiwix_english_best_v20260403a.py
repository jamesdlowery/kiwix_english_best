    ...
    if choice == "1":
        ...
        if os.path.exists(RESUME_FILE):
            ...
            if resume_items:
                ...
                if resume_choice == "r":
                    required_gb = get_required_space_gb(resume_items)
                    available_gb = get_free_space_gb(ZIM_SUBFOLDER)
                    print(f"\nSpace check:")
                    print(f"  Required (max, incl. buffer): ~{bytes_to_binary_human(int(required_gb * 1024**3))}")
                    print(f"  Available: ~{bytes_to_binary_human(int(available_gb * 1024**3))}")
...
#
# Changes in this version (option 4 preview and confirmation before deletion):
#
# FIX 53 -- cleanup_zims_directory() now shows a full preview of what will
#   be kept and deleted before asking for confirmation, rather than deleting
#   immediately. The preview lists every duplicate group, which file will be
#   kept, and which file(s) will be deleted with their sizes. The total space
#   to be freed is shown, then the user is prompted:
#
#     Proceed with deletion? (y/n):
#
#   Answering 'n' cancels without touching any files. Answering 'y' proceeds
#   with deletion exactly as before. The same two-pass approach (preview then
#   delete) is used so the groups are only iterated twice — once to build the
#   preview, once to perform the deletions.
#
# Earlier fixes from v20260402l carried forward:
#
# FIX 52 (revised) -- size_human is now the 2nd field, immediately after
#   filename, making list files easier to read at a glance:
#
#   filename|size_human|size_bytes|url|torrent_url
#
#   Example:
#   wikipedia_en_all_maxi_2026-02.zim|115.00 GiB|123456789012|https://...|https://...
#
#   load_list() auto-detects the format by checking whether parts[1] parses
#   as an integer: if yes, old format (size_bytes at index 1); if no, new
#   format (size_human at index 1, size_bytes at index 2). This ensures
#   existing list files from earlier versions still load correctly.
#   Both save_list() and save_resume_file() use the new format.
#
# Earlier fix from v20260402j carried forward:
#
# FIX 52 -- save_list() and save_resume_file() now write a 5th pipe-delimited
#   field, size_human, containing the human-readable file size (e.g. "2.30 GiB").
#   The format comment in the header is updated to reflect the new field.
#   load_list() already ignores fields beyond index 3, so existing files with
#   only 4 fields load correctly and new files with 5 fields also load correctly.
#   size_bytes (field 2) is unchanged and still drives all calculations.
#
#   Before: wikipedia_en_all_maxi_2026-02.zim|123456789|https://...|https://...
#   After:  wikipedia_en_all_maxi_2026-02.zim|123456789|https://...|https://...|115.00 GiB
#
# Earlier fix from v20260402j carried forward:
#
# FIX 51 -- Option 1 now always saves the full best list to kiwix_english_best.txt,
#   regardless of whether any files need downloading. Two bugs fixed:
#
#   (a) save_list() was gated on a "Save list to file? (y/n)" prompt and was
#       skipped entirely if the user answered "n" or if everything was already
#       downloaded. The list file now always reflects the current best selection
#       from the server — the prompt has been removed.
#
#   (b) filter_existing_files() was called before save_list(), so the saved
#       list only contained files not yet on disk — not the full best selection.
#       Now save_list() is called on the full best list first, then
#       filter_existing_files() produces a separate to_download list used only
#       for the download step. If to_download is empty the script reports that
#       all files are present and exits, but the list file has still been saved.
#
# Earlier fix from v20260402i carried forward:
#
# FIX 50 -- Added -u / --update command-line flag for non-interactive use.
#   When invoked with -u or --update, the script runs a full option 3 update
#   cycle without any prompts — suitable for cron jobs or automation.
#
#   Behaviour:
#   - Exits silently with code 0 if ZIM_SUBFOLDER doesn't exist (VeraCrypt
#     volume not mounted), so cron doesn't generate spurious failure emails.
#   - If kiwix_resume_update.txt exists, resumes the incomplete session.
#   - Otherwise crawls the server, downloads all updates and new groups,
#     with cleanup enabled and torrent support.
#   - Non-verbose output (clean for log files).
#   - Exits with code 1 if space is insufficient or the crawl fails.
#
#   Example cron entry (every Sunday at 3am):
#     0 3 * * 0  python3 /path/to/kiwix_english_best.py -u >> /path/to/kiwix_cron.log 2>&1
#
# Earlier fix from v20260402h carried forward:
#
# The save_list() refactoring in v20260402g accidentally dropped the
# `def log_failure(...)` line, leaving the function body as bare module-
# level code between delete_resume_file() and verify_zim_integrity(). After
# a successful resume session, delete_resume_file() returned normally and
# Python executed the orphaned log_failure body, hitting `via_torrent` which
# was not defined in module scope — causing a NameError crash. The download
# itself completed successfully; only the post-session cleanup crashed.
# Fixed by restoring the missing def line.
#
# Earlier fixes from v20260402g carried forward:
#
# FIX 49 -- Option 3 (updates) now uses torrents where available, matching
#   the behaviour of options 1 and 2. Previously, the items returned by
#   check_for_updates() had no torrent_url field because they came from
#   fetch_directory() rather than going through save_list(). The torrent-
#   checking logic has been extracted from save_list() into a new standalone
#   helper add_torrent_urls(items) which mutates items in-place. save_list()
#   now delegates to add_torrent_urls(). Option 3 calls add_torrent_urls()
#   on to_download (and on resume_items in the resume path) immediately
#   before calling download_list(), so download_single can attempt a torrent
#   download first and fall back to HTTP if unavailable.
#
# Earlier fixes from v20260402f carried forward:
#
# FIX 48 -- Two improvements:
#
#   (a) Option 3 now detects kiwix_resume_update.txt at startup. If the file
#       exists the user is shown the incomplete files and offered a choice:
#       (r) resume the previous update session directly, or (n) re-crawl the
#       server and check for all updates. Choosing 'r' skips the server crawl
#       entirely and resumes only the incomplete items. The resume file is
#       written/deleted inside download_list's finally block (via the new
#       resume_file= parameter) so it is always in sync with the actual state
#       of partial files on disk. The post-run summary now correctly says
#       "Run option 3 to resume" (not option 2) when a resume file was saved.
#
#   (b) download_http now captures and returns the last error as a string
#       alongside the boolean success flag (returns a tuple). Specific
#       exception types are caught separately: HTTPError (with status code
#       and reason), Timeout, ConnectionError, and generic Exception. The
#       ✗ Failed inline message now appends the error description so the user
#       knows why the download failed (e.g. "HTTP 503: Service Unavailable",
#       "Connection timed out", "Size mismatch: got X, expected Y").
#
# Earlier fixes from v20260402e carried forward:
#
# The inline ✗ Failed block previously printed "Run option 2 to resume this
# download." immediately after reporting the partial file. The post-run summary
# already lists all incomplete files and prints "→ Run option 2 to resume these
# downloads." as the single authoritative location for that instruction. Having
# it in both places made it look like the file was being reported twice (once
# as a failure and once as an incomplete download), which was confusing. The
# inline failure message now only reports the filename and partial file path;
# the post-run summary handles the resume instruction.
#
# Earlier fixes from v20260402d carried forward:
#
# Renamed kiwix_resume.txt → kiwix_resume_update.txt for clarity.
# RESUME_FILE constant updated accordingly.
#
# Earlier fixes from v20260402c (FIX 47 and prior) carried forward:
#
# FIX 47 -- Option 3 now writes a resume file (kiwix_resume_update.txt) after a
#   download session if any files did not complete. The file follows the same
#   format as the main list file and contains only the incomplete items. On
#   the next option 2 run, if kiwix_resume_update.txt exists the user is offered the
#   choice to resume from it rather than the main list. Once all items in the
#   resume file complete successfully, the file is deleted automatically. If
#   some items still fail, the resume file is rewritten with the remaining
#   incomplete items so it stays current. If the option 3 session completes
#   with no partial files remaining, any existing resume file is deleted.
#   New helpers: save_resume_file(), delete_resume_file().
#   New constant: RESUME_FILE = "kiwix_resume_update.txt"
#
# Earlier fixes from v20260402b carried forward:
#   genuine partial reporting with remaining bytes):
#
# FIX 46 -- Four related display and reporting improvements:
#
#   (a) Startup lines (↓/↻) now printed in file-index order as a single
#       block before any progress output begins. Each download_single thread
#       deposits its startup message into the global startup_lines dict keyed
#       by file_index. The refresh thread waits until all total_files entries
#       are collected, prints them in sorted order, prints a blank line, then
#       proceeds to normal progress display.
#
#   (b) Progress lines in the refresh block are now sorted by file_index so
#       they always appear in the same order as the startup lines regardless
#       of thread scheduling. file_index is stored in every active_downloads
#       entry and passed through to both download_http and download_torrent.
#
#   (c) On successful download + verification, the active_downloads entry
#       is updated to state='Completed', progress=100.0 before removal.
#       The refresh thread renders one final "Completed | 100.0%" line for
#       the file, then it is removed after a short sleep. Previously the
#       entry was popped immediately, leaving a stale "Downloading" state
#       or no line at all in the final snapshot.
#
#   (d) Post-run summary partial reporting overhauled:
#       - Only genuine incomplete files are reported (partials where no
#         corresponding .zim file exists -- filters out files that completed
#         and were renamed after the refresh thread last checked).
#       - Each incomplete file now shows bytes on disk AND bytes remaining
#         (expected − on disk), calculated from the items list.
#       - "Run option 2" instruction appears after a blank line, only when
#         there are actual incomplete files.
#
# Earlier fix from v20260402a carried forward:
#
# FIX 45 -- In non-verbose mode the refresh thread uses ANSI cursor-up to
#   rewrite the progress block in place. When the session ended, the post-run
#   summary was printed without first breaking out of the rewrite area, causing
#   the cursor-up sequences to overwrite the first lines of the summary and
#   leave only the last line (e.g. "Run option 2 to resume this download.")
#   visible on screen with no context. Fixed by flushing stdout and writing
#   two blank lines before the summary to advance the cursor below the last
#   in-place block. Also removed the leading \n from "\nPost-run summary:"
#   since the blank lines now provide the visual separation.
#
# Earlier fixes from v20260401l carried forward:
#
# FIX 44 -- Two improvements to make partial download failures visible:
#
#   (a) download_single: when a download fails and a .partial file exists,
#       the failure message now reports the partial file path and size and
#       instructs the user to run option 2 to resume. Previously "Failed:"
#       was printed with no indication that partial data was preserved or
#       what to do next. Also fixed a cosmetic bug where file_index and
#       total_files could appear garbled in the Failed line.
#
#   (b) Post-run summary in download_list: after every session, zims/ is
#       scanned for .partial files. If any are found they are listed with
#       their on-disk size and a reminder to run option 2 to resume. This
#       ensures partial downloads are visible even if the failure message
#       scrolled off screen or the terminal was closed before it was read.
#
# Earlier fix from v20260401k carried forward:
#
# FIX 43 -- select_best_per_group(): sort key changed from
#   (priority, -size, -date) to (priority, -date, -size).
#   Previously size was ranked before date, so an older but slightly larger
#   file could beat a newer but smaller one. For example, the server had both
#   devdocs_en_axios_2025-10.zim (407 KiB) and devdocs_en_axios_2026-02.zim
#   (330 KiB); the 2025-10 version was selected as "best" due to its larger
#   size, causing option 3 to never detect the 2026-02 version as a new group
#   or update. Date is now the primary tiebreaker after structural priority,
#   with size as a secondary tiebreaker for files of the same date.
#
# Earlier fixes from v20260401j carried forward:
#
    ...
    if args.update:
        ...
        if os.path.exists(RESUME_FILE):
            ...
            if resume_items:
                ...
                available_gb = get_free_space_gb(ZIM_SUBFOLDER)
                print(f"\nSpace check:")
                print(f"  Required: ~{bytes_to_binary_human(int(required_gb * 1024**3))}")
                print(f"  Available: ~{bytes_to_binary_human(int(available_gb * 1024**3))}")
                if available_gb < required_gb:
