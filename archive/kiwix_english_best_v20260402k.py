...
def save_list(items, filename=LIST_FILE):
    ...
    with open(filename, 'w', encoding='utf-8') as f:
        ...
        # FIX 52: size_human added as a 5th field for readability when the
        # file is opened in a text editor. size_bytes (field 2) is still used
        # for all calculations; size_human is informational only.
        f.write("# Format: filename|size_bytes|url|torrent_url|size_human\n\n")
        for item in items:
            size_human = bytes_to_binary_human(item['size_bytes'])
            f.write(f"{item['filename']}|{item['size_bytes']}|{item['url']}|{item.get('torrent_url', '')}|{size_human}\n")
        ...
        for line in f:
            ...
            if len(parts) >= 3:
                ...
                try:
                    sz = int(parts[1])
                    http_url = parts[2]
                    # torrent_url is field 4 (index 3); size_human field 5
                    # (index 4) is ignored on load — size_bytes drives all math.
...
#
# Changes in this version (human-readable size in list files):
#
