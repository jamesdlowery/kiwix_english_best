...
def total_bar_refresh_thread(pbar, total_expected, stop_event, verbose):
    ...
    while not stop_event.is_set():
        ...
        # FIX 9: Progress position uses total_bytes_on_disk (includes partial
        # baseline) so the bar starts at the correct position on resume.
        pbar.n = min(total_bytes_on_disk, total_expected)
        pbar.refresh()

        # FIX 9: Speed and ETA use total_bytes_this_session (session-only bytes)
        # so the rate reflects current network throughput, not an average
        # diluted by however long the original download took.
        if elapsed > 3.0:
...
#
# Changes in this version (progress position / speed / ETA fully decoupled):
#
# FIX 9 -- Replaced single total_bytes_downloaded counter with two independent
#   counters that serve different purposes:
#
#   total_bytes_on_disk:      seeded from partial files at startup + all new
#                             bytes written during the session. Drives the
#                             progress bar position only. On resume, the bar now
#                             correctly starts partway through rather than at 0%.
#
#   total_bytes_this_session: always starts at 0, incremented only by bytes
#                             genuinely received from the network in the current
#                             session. Drives speed and ETA only, so the rate
#                             reflects current throughput rather than an average
#                             diluted by prior sessions or disk verification.
#
#   Both counters are updated in download_http (all HTTP bytes are network
#   bytes, so both always increment together) and download_torrent (on_disk
#   increments always; this_session only increments outside CHECKING_STATES).
#   The refresh thread and wrapped_download completion handler were updated to
#   use the appropriate counter for each purpose.
#
...
#   FIX 7 -- torrent byte counting gated on network state (superseded by FIX 9)
#   FIX 8 -- partial file baseline seeding (superseded by FIX 9)
