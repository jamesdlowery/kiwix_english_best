...
total_bytes_downloaded = 0   # Global counter for real-time total progress
...
# ==================== TOLERANT FILTER FOR EXISTING FILES ====================
                ...
                with open(target_path, mode) as f:
                    for chunk in r.iter_content(chunk_size=CHUNK_SIZE):
                        ...
                        if chunk:
                            f.write(chunk)
                            written += len(chunk)
                            total_bytes_downloaded += len(chunk)   # REAL-TIME TOTAL UPDATE
                            if verbose:
...
def download_list(items, verbose=False):
    global active_downloads, total_bytes_downloaded
...
# - Version bumped to v20260327a to reflect current date
# - Real-time total progress bar based on actual bytes downloaded (torrent + HTTP)
# - Tolerant duplicate filtering with clear diagnostics
# - Two decimal places and proper unit scaling everywhere
# - Wiktionary exemption for wiktionary_en_all_nopic_*
# 
# This is the production-ready version with live total progress and all requested features.
