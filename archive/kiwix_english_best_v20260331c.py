...
def total_bar_refresh_thread(pbar, total_expected, stop_event, verbose):
    ...
    while not stop_event.is_set():
        ...
        if now - last_print >= TOTAL_BAR_REFRESH_SECONDS:
            ...
            if verbose:
                # FIX 27: On transition, leave last verification block in
                # place (it already scrolled), print a blank line separator,
                # then start fresh download blocks below.
                ...
                # FIX 27: On transition in non-verbose mode, stop overwriting
                # the previous block (leave it visible), print a blank line,
                # then reset last_block_lines so the next cycle starts fresh.
                if pending_transition_blank:
                    sys.stdout.write("\n")
                    sys.stdout.flush()
                    last_block_lines = 0
...
#
# Changes in this version (fire clock reset only on first file's transition):
#
