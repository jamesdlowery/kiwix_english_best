...
Version: v20260325j   # FIXED: TOTAL progress bar now prints after each batch in verbose mode
"""
...
def download_list(items, verbose=False):
    ...
    pbar_total = tqdm(total=total_bytes_expected, desc="Total progress", unit='B', unit_scale=True, unit_divisor=1024,
                      disable=not TQDM_AVAILABLE, leave=True)
    ...
    try:
        def wrapped_download(item, idx):
            ...
            if success:
                ...
                # Print TOTAL progress bar after each batch in verbose mode
                if verbose:
                    print(pbar_total)
