...
- Wikipedia: keep ONLY wikipedia_en_all_maxi_* (drop everything else including simple_all_maxi, mini, nopic, topic, top, 100, etc.)
- All other projects: keep _all_maxi_ / _all_ when present; drop ALL _nopic_ versions
- Drop wikivoyage_en_europe_*, speedtest_*, and any other _simple_all_* that is not the main Wikipedia one
"""
...
# ==================== TIGHT EXCLUSION FILTER ====================
def should_exclude(filename):
    lower = filename.lower()

    if 'speedtest' in lower:
        return True
    # Gutenberg: drop every lcc split
    ...
    if lower.startswith('wikipedia_en_'):
        if '_all_maxi_' not in lower or '_simple_all_' in lower:
            return True
        return False
    # Drop ALL _nopic_ versions everywhere
    if '_nopic_' in lower:
        return True
    # Drop specific unwanted variants
