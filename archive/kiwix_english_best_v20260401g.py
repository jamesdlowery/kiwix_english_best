    ...
    try:
        def wrapped_download(item, idx):
            ...
            if success:
                ...
                # FIX 38: If cleanup was requested, remove older versions of
                # this file immediately after it completes rather than waiting
                # until all downloads finish. This ensures cleanup happens even
                # if the session is interrupted before other files complete.
...
def check_for_updates():
    ...
    # FIX 41: Also scan zims/corrupt/ so files that failed verification
    # aren't re-listed as new groups on the next option 3 run.
    on_disk = {}
    scan_dirs = [ZIM_SUBFOLDER]
    if os.path.exists(CORRUPT_SUBFOLDER):
        scan_dirs.append(CORRUPT_SUBFOLDER)
    ...
    for key, disk_item in on_disk.items():
        ...
        # Even a 5% threshold produces false positives: a 153 MiB file parsed
        # from "153M" vs os.path.getsize() can differ by enough to exceed the
        # threshold, flagging same-version files as updates. Content dates from
        # filenames are unambiguous and sufficient for update detection.
...
#
# Changes in this version (fix false update positives in option 3):
#
# FIX 41 -- Two fixes to check_for_updates() to eliminate false positives:
#
#   (a) Removed the size threshold comparison (curr_size > disk_size * 1.05).
#       Server sizes come from approximate Apache directory strings (e.g. "153M",
#       "5.2G") which parse to rounded values, while on-disk sizes are exact byte
#       counts from os.path.getsize(). The 5% threshold was not wide enough to
#       absorb this discrepancy, causing same-version files to be flagged as
#       updates (e.g. citizensinformation 2025-12 vs 2025-12, axios 2025-10 vs
#       2025-10). Update detection now relies solely on content date from filename.
#
#   (b) on-disk scan now also covers zims/corrupt/ in addition to zims/. Files
#       that failed verification and were moved to corrupt/ were invisible to the
#       scan, causing them to be re-listed as "new groups" on the next option 3
#       run (e.g. coreyms_en_pandas-tutorials listed as new despite being in
#       corrupt/). Both directories are now scanned; newest date wins per group.
#
