...
def total_bar_refresh_thread(pbar, total_expected, stop_event, verbose):
    global total_bytes_on_disk, total_bytes_this_session
    global download_start_time, display_start_time, reset_session_clock
    last_print = 0
    ...
    while not stop_event.is_set():
        ...
        # transition, reset the speed/ETA clock and session byte counter so that
        # speed and ETA start fresh from the moment downloading begins.
        # display_start_time is NOT reset here -- it keeps counting from session
        # start so the elapsed display shows total session time continuously.
        if reset_session_clock:
            total_bytes_this_session = 0
            download_start_time = now
            reset_session_clock = False

        # FIX 26: elapsed for display comes from display_start_time (never
        # resets) so it counts up throughout verification and downloading.
        # elapsed for speed/ETA comes from download_start_time (resets on
        # checking->downloading transition) so it reflects only download time.
...
def download_list(items, verbose=False):
    ...
    display_start_time  = time.time()   # never resets -- drives elapsed display
    download_start_time = time.time()   # resets on transition -- drives speed/ETA
    reset_session_clock = False
...
#
# Changes in this version (elapsed counts during verification; resets to zero on download start):
#
