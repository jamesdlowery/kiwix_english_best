...
Version: v20260325l   # FIXED: Total progress bar now reliably refreshes every ~5s + after each file in verbose mode
"""
...
TOTAL_BAR_REFRESH_SECONDS = 5.0      # NEW: how often to force total bar in verbose mode
...
# ... [All helper functions from is_english_zim through get_summary_stats remain exactly the same as in v20260325k] ...
...
# ==================== CRAWLER, SAVE/LOAD, INTEGRITY, DOWNLOAD FUNCTIONS (unchanged) ====================
# [All functions from fetch_directory through download_http and download_torrent remain identical to v20260325k]
...
# ==================== TOTAL BAR REFRESH THREAD (NEW) ====================
def total_bar_refresh_thread(pbar, stop_event, verbose):
    while not stop_event.is_set():
        if verbose:
            print(pbar)                     # Force the total progress line
...
def download_list(items, verbose=False):
    ...
    # Start total bar refresh thread for verbose mode
    if verbose:
