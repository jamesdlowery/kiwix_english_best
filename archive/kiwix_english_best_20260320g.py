...
Version: v20260320g   # Total bar = bytes + visible 'Files: X / Y' text line after each file
...
# ==================== DOWNLOAD WITH DUAL PROGRESS BARS + FILE COUNT DISPLAY ====================
def download_file(item, file_index, total_files, pbar_total, current_total_bytes):
    url = item['url']
    ...
    if os.path.exists(local_path):
        ...
        if local_size == expected_bytes:
            ...
            pbar_total.update(expected_bytes)
            ...
            with open(local_path, mode) as f:
                with tqdm(
                    ...
                    while True:
                        ...
                        pbar_file.update(chunk_len)
                        pbar_total.update(chunk_len)

        print(f"✓ Completed: {filename} ({file_index}/{total_files})")
        return True, bytes_added
...
def download_list(items):
    ...
    print(f"\nStarting download of {total_files} files ({bytes_to_human(total_bytes_expected)} total)... (Ctrl+C to pause)\n")

    remaining = items[:]

    with tqdm(
        total=total_bytes_expected,
        initial=total_bytes_done,
        desc="Total",
        unit='B',
        unit_scale=True,
        unit_divisor=1024,
        ...
        try:
            for i, item in enumerate(items, 1):
                success, bytes_added = download_file(item, i, total_files, pbar_total, total_bytes_done)
                total_bytes_done += bytes_added
                if success:
                    remaining.remove(item)
                    save_state(remaining)
                    # Show current file count after completion
                    print(f"Files completed: {i} / {total_files} ({bytes_to_human(total_bytes_done)} total)\n")
        except KeyboardInterrupt:
    ...
    print(f"\n🎉 All {total_files} files completed! ({bytes_to_human(total_bytes_done)} downloaded)")
