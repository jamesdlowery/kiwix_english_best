...
Version: v20260319c   # Added total file count & total size in saved file header
Purpose: Recursively crawl https://download.kiwix.org/zim/, find English ZIM files,
         parse real dates & sizes from Apache listings, select latest + largest per variant.
...
def main():
    ...
    if not all_english:
        print("No English ZIM files found – check connection, server format, or English patterns.")
        return
    ...
    if save == 'y':
        ...
        total_size_parts = [f["size_str"] for f in best_files if f["size_str"] != '-']
        total_size_str = " + ".join(total_size_parts) if total_size_parts else "unknown"
        ...
        with open('kiwix_english_best.txt', 'w', encoding='utf-8') as f:
            ...
            f.write(f"# Total estimated download size: ~{total_size_str}\n")
            f.write("# Note: Sizes are as reported by server; actual may vary slightly.\n\n")
            f.write("\n".join(best_urls) + "\n")
        print("Saved to kiwix_english_best.txt (with totals in header)")
